...
import { importDisks } from './importDisks.mjs'
...
export default class MigrateVm {
  ...
  async connectToEsxiAndList({ host, user, password, sslVerify, vmId }) {
  ...
  async migrationfromEsxi(
    ...
    const vhds = await importDisks({ esxi, dataStoreToHandlers, vm, chainsByNodes: coldChainsByNodes, sr, vmId })
    if (isRunning && stopSource) {
      await importDisks({ esxi, dataStoreToHandlers, vm, chainsByNodes: runningChainByNodes, sr, vmId, vhds })
    }
