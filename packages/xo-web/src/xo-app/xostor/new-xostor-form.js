...
const doesNetworkHasPifs = network => network.PIFs.length > 0
const doesPifsHaveIp = pifs =>
  pifs.every(pif => pif.ip !== '' || (pif.ipv6.length > 0 && pif.ipv6.some(v6 => v6 !== '')))
    ...
    computed: {
      networksPredicate: (state, props) => network => {
        ...
        return state.onlyShowXostorNetworks
          ? isOnPool &&
              doesNetworkHasPifs(network) &&
              arePifsStatic(pifs) &&
              doesPifsHaveIp(pifs) &&
              arePifsAttached(pifs)
          : isOnPool
      },
      networkHasPifs: (state, props) => doesNetworkHasPifs(props.networks[state.networkId]),
      ...
      pifsHaveIp: (state, props) => doesPifsHaveIp(props.pifsByNetwork[state.networkId]),
      isInterfaceNameContainsWhiteSpace: state => state.interfaceName.includes(' '),
      isInterfaceNameReserved: state => state.interfaceName.trim() === 'default',
      isNetworkCompatible: state =>
        state.networkHasPifs && state.pifsAreStatic && state.pifsHaveIp && state.pifsAreAttached,
    },
  }),
  injectState,
  ({ effects, state }) => (
    <Card>
      ...
      <CardBlock>
        ...
        <Row className='mb-1'>
          <Col>
            ...
            <ul className='text-danger'>
              {state.isInterfaceNameReserved && (
                <li>
                  <Icon icon='alarm' /> {_('interfaceNameReserved')}
                </li>
              )}
              {state.isInterfaceNameContainsWhiteSpace && (
                <li>
        ...
        />
        {state.networkId !== undefined && !state.isNetworkCompatible && (
          <div className='text-danger'>
            <ul>
              {!state.networkHasPifs && <li>{_('networkNoPifs')}</li>}
